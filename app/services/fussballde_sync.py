...
def sync_match(session: Session, source: FussballDeSource, match_import: MatchImport) -> Match:
    ...
    monitored_entries = [
        entry for entry in lineup_entries if entry.side == match_import.monitored_team_side
    ]
    ...
    player_names = _resolve_player_names(source, monitored_entries)

    return import_match(session, match_import, lineup_entries, player_names)
